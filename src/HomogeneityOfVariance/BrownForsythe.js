import { sum, mean, median } from 'simple-statistics';
...
export default function BrownForsythe({ observed, alpha = 0.05, digit = 4, way = 'one-way' }) {
	...
	const medianObserved = Array.from({ length: c }, (_, i) => median(observed[i]));
	const Z = Array.from({ length: c }, (_, i) => {
		return Array.from({ length: ni[i] }, (_, j) => Math.abs(observed[i][j] - medianObserved[i]));
	});
